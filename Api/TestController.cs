...
namespace Inmobiliaria_2022.Api
{
	...
	public class TestController : ControllerBase
	{
		...
		{
			if(id > 0)
				return Ok(Contexto.Propietario.Where(x => x.IdPropietario == id).Select(x => x.Email).Single());
			else
		...
		{
			return id > 0 ?
				Ok(Contexto.Propietario.Where(x => x.IdPropietario == id)
				.Select(x => new { Id = x.IdPropietario, x.Email }).Single()) :
				Ok(Contexto.Propietario.Select(x => new { Id = x.IdPropietario, x.Email }).ToList());
		}
